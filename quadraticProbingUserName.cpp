...
int main() {
  ...
  
    {
        ...
        for (long long j = 0; j < 20000 && j < usuarios.size(); ++j) {
            quadraticProbing.searchn(usuarios[j].user_name);
        }
        auto end = chrono::high_resolution_clock::now();
        chrono::duration<double> duration = end - start;
        cout << "Tiempo para buscar usuarios en quadraticProbingUserName: " << duration.count() << " segundos" << endl;
    }

    // Insertar usuarios y medir el tiempo
    for (int i = 0; i < num_inserciones; ++i) {
...

        {
            ...
            for (long long j = 0; j < inserciones_size && j < usuarios.size(); ++j) {
                quadraticProbing.insertn(usuarios[j].user_name, usuarios[j]);
            }
            auto end = chrono::high_resolution_clock::now();
            chrono::duration<double> duration = end - start;
            cout << "Tiempo para insertar " << inserciones_size << " usuarios en quadraticProbing (userName): " << duration.count() << " segundos" << endl;
        }
    }
